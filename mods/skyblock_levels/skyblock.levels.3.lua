...
skyblock.levels[level].init = function(player_name)
   core.chat_send_player(player_name, "You can now choose your own skin")
end
...
skyblock.levels[level].bucket_water_on_use = function(player_name, pointed_thing)
	...
	for _, dpos in pairs({
		...
		if minetest.get_node({x=pos.x+dpos[1], y=pos.y, z=pos.z+dpos[2]}).name=='default:water_source'
		and (minetest.get_node({x=pos.x+dpos[3], y=pos.y, z=pos.z+dpos[4]}).name=='air'
		or minetest.get_node({x=pos.x+dpos[3], y=pos.y, z=pos.z+dpos[4]}).name=='default:water_flowing') then
			skyblock.feats.add(level,player_name,'place_water_infinite')
			return
		end
	end
