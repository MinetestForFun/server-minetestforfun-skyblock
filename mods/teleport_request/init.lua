...
-- Modified to suit MFF's Skyblock server
...
local function tpc_send(player,coordinates)
	...
	if minetest.check_player_privs(pname, {tp_admin=true}) then
		...
		if minetest.check_player_privs(pname, {tp_tpc=true}) then
			...
			if protected then
				if not protector.can_dig(1, target_coords, player, false, 0) then
					minetest.chat_send_player(player,("Error: %s is protected by %s."):format(minetest.pos_to_string(target_coords), minetest.get_meta(target_coords):get_string("owner")))
					return
				end
			end
			minetest.chat_send_player(player, 'Teleporting to '..posx..','..posy..','..posz)
