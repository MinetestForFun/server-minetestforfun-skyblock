	...
	elseif _doors.registered_doors3[minetest.get_node(pos).name] then --MFF doors3
		...
		return {
			...
			open = function(self, player)
				...
				return _doors.door3_toggle(self.pos, player)
			end,
			close = function(self, player)
				if not self:state() then
					return false
...
doors.register("door_cherry", {
		tiles = {{ name = "doors_door_wood_cherry.png", backface_culling = true }},
...
doors.register("door_prison", {
		tiles = {{ name = "doors_door_prison.png", backface_culling = true }},
...
-- MFF doors 3 nodes
function _doors.door3_toggle(pos, clicker)
	local meta = minetest.get_meta(pos)
	local state = meta:get_int("state")
	local def = minetest.registered_nodes[minetest.get_node(pos).name]
	local name = def.door.name
...
function doors.register3(name, def)
	...
	minetest.register_abm({
		...
		action = function(pos, node, active_object_count, active_object_count_wider)
			...
			-- wipe meta on top node as it's unused
			minetest.set_node({x = pos.x, y = pos.y + 1, z = pos.z}, { name = "doors:hidden" })
			minetest.set_node({x = pos.x, y = pos.y + 2, z = pos.z}, { name = "doors:hidden" })
		end
	})
	...
	def.on_rightclick = function(pos, node, clicker)
		_doors.door3_toggle(pos, clicker)
	end
	def.after_dig_node = function(pos, node, meta, digger)
		minetest.remove_node({ x = pos.x, y = pos.y + 1, z = pos.z})
...
doors.register3("door3_cherry", {
		tiles = { "doors_door3_wood_cherry.png" },
		description = "Cherry Door 3 Nodes",
		inventory_image = "doors3_item_wood_cherry.png",
		groups = {choppy=2, oddly_breakable_by_hand=2, flammable=2},
		sounds = default.node_sound_wood_defaults(),
		recipe = {
			{"", "", ""},
			{"", "doors:door_wood_cherry", ""},
			{"", "doors:door_wood_cherry", ""},
		},
})

doors.register3("door3_prison", {
		tiles = {{ name = "doors_door3_prison.png", backface_culling = true }},
