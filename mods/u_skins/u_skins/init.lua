	...
	get_formspec = function(player)
		...
		if u_skins.get_type(u_skins.u_skins[name]) == u_skins.type.MODEL then
			formspec = formspec
				...
				--.. "image[1,.75;1,2;"..u_skins.u_skins[name].."_preview_back.png]" -- Disabled until checked
...
for x = 0, math.floor(#u_skins.list/16+1) do
	unified_inventory.register_page("u_skins_page_"..x, {
		get_formspec = function(player)
			...
			-- prev next page buttons -- Modified (Mg|01/15/16)
			--if page > 0 then
				formspec = formspec .. "button[0,4;1,.5;u_skins_page_"..(page-1)%(math.floor(#u_skins.list/16+1))..";<<]"
			--else
			...
			--if index > 16 then
				formspec = formspec .. "button[7,4;1,.5;u_skins_page_"..(page+1)%(math.floor(#u_skins.list/16+1))..";>>]"
			--else
